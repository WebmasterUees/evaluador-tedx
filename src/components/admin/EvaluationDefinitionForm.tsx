...
export default function EvaluationDefinitionForm({ participants, evaluators, groups, creators, groupParticipantsMap, action }: Props) {
    ...
    <form action={action} className="space-y-5 rounded-2xl bg-white p-6 shadow-sm">
      <div className="grid gap-4 md:grid-cols-2">
        <label className="block">
          ...
          <input name="title" required className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm outline-none transition placeholder:text-slate-400 focus:border-slate-400 focus:ring-2 focus:ring-slate-200" placeholder="Ej. Evaluación de ponencias - ESTUDIANTES" />
        </label>
        <label className="block">
      ...
      <fieldset className="space-y-2 rounded-xl border border-slate-200 p-4">
        ...
        <p className="px-2 text-xs text-slate-500">Marca uno o varios evaluadores para esta evaluación.</p>
        <div className="grid gap-2 sm:grid-cols-2 md:grid-cols-3">
          {evaluators.map((evaluator) => (
            <label key={evaluator.id} className="inline-flex items-center gap-2 text-sm text-slate-700">
              <input type="checkbox" name="evaluator_ids" value={evaluator.id} className="h-4 w-4" />
              {evaluator.name}
            </label>
          ))}
        </div>
      ...
      <fieldset className="space-y-3 rounded-xl border border-slate-200 p-4">
        <legend className="px-2 text-sm font-semibold text-slate-700">Preguntas y escala de puntuación</legend>
        <p className="px-2 text-sm text-slate-500">Define el texto de cada pregunta, su peso en el cálculo final y el rango de puntuación permitido.</p>
